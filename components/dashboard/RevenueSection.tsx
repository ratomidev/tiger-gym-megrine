...
export default async function RevenueSection() {
  ...
  const revenueChangePercent = totalRevenuePrevMonth > 0
  ...
  const projectedChangePercent = totalRevenueThisMonth > 0
    ? ((projectedRevenueNextMonth - totalRevenueThisMonth) / totalRevenueThisMonth) * 100
    : 100;

  // ---------------------------------------------------------------
  // 3. Prix moyen d'abonnement
  // ---------------------------------------------------------------
  const averagePrice = currentMonthSubscriptions.length > 0
    ? totalRevenueThisMonth / currentMonthSubscriptions.length
    : 0;
  
  // Obtenir la moyenne du mois précédent pour comparaison
  const prevAveragePrice = prevMonthSubscriptions.length > 0
    ? totalRevenuePrevMonth / prevMonthSubscriptions.length
    : 0;
  
  // Calculer le pourcentage de changement du prix moyen
  const averagePriceChangePercent = prevAveragePrice > 0
  ...
  const planBreakdown = subscriptionsByPlan
    .map(plan => ({
      ...
      percentage: totalRevenueThisMonth > 0
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex items-center pt-1">
              ...
              <Badge variant={revenueChangePercent >= 0 ? "outline" : "destructive"}>
                {Math.abs(revenueChangePercent).toFixed(1)}%
              </Badge>
              <span className="text-xs text-muted-foreground ml-2">par rapport au mois dernier</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              Revenu Prévisionnel ({format(addMonths(now, 1), 'MMMM yyyy', { locale: fr })})
            </CardTitle>
            <IconCash className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(projectedRevenueNextMonth)}</div>
            <div className="flex items-center pt-1">
              ...
              <Badge variant={projectedChangePercent >= 0 ? "outline" : "destructive"}>
                {Math.abs(projectedChangePercent).toFixed(1)}%
              </Badge>
              <span className="text-xs text-muted-foreground ml-2">vs. mois actuel</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex items-center pt-1">
              ...
              <Badge variant={averagePriceChangePercent >= 0 ? "outline" : "destructive"}>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex items-center pt-1">
              {currentMonthSubscriptions.length - prevMonthSubscriptions.length >= 0 ? (
                <IconArrowUpRight className="mr-1 h-4 w-4 text-green-500" />
              ) : (
                <IconArrowDownRight className="mr-1 h-4 w-4 text-red-500" />
              )}
              <Badge 
                variant={currentMonthSubscriptions.length - prevMonthSubscriptions.length >= 0 
                  ? "outline" 
                  : "destructive"}
              >
                {Math.abs(currentMonthSubscriptions.length - prevMonthSubscriptions.length)}
              </Badge>
              <span className="text-xs text-muted-foreground ml-2">par rapport au mois dernier</span>
            </div>
          </CardContent>
